...
1. 🟡 **Nemotron Speech Streaming via sherpa-onnx** _(bench-grader landed 2026-05-21 — `5ecd9b6`)._ NVIDIA Nemotron Speech Streaming en-0.6B is wired as an addressable STT engine via a new `stt::online_transducer` FFI wrapper over sherpa-onnx's online recognizer (sherpa-rs 0.6.8 only ships the offline path in safe Rust; the streaming graph's encoder takes cache tensors so we drive the C side directly through `sherpa-rs-sys`). `bench_replay` discovers the bundle alongside Parakeet and Whisper. Pull with `pnpm download:model:mac nemotron`. **Still open**: (a) actual bench numbers on the existing capture set — gates the default swap per the "net improvement on both speed and quality" rule; (b) production-side streaming integration (chunked accept_waveform + partial polling) for live preview — currently mirrors the offline `SttEngine` shape so the bench can grade Nemotron without refactoring the trait. License note: NVIDIA Open Model License Agreement (commercial OK, not Apache/MIT) — add NOTICE entry when this becomes a default.
   - Companion bench rows: 🟡 `qwen3:4b` / `qwen3:8b` / `qwen3:1.7b` LLM candidates added to the same harness (`687eacf`). Survey-recommended candidate is `qwen3:4b-instruct-2507` (Pareto-dominates `qwen2.5:7b` on the IFEval-vs-size axis); the 8B and 1.7B rows are the "quality stretch if tok/s is at least neutral" data points. `pnpm ollama:pull:bench` pulls all five up front.
