...
**Phases (status as of 2026-05-21):**

0. ✅ **Style overhaul** _(shipped — `d71cb90` + `4ba7e95`; revised 2026-05-10)_. Replaced the tone-based system (casual / formal / very-casual / excited) with a **structuring-aggressiveness axis** — original landing was raw / light / moderate / assertive; the 2026-05-09 graded-bench round (20 variants, single capture) showed Assertive hallucinating entire paragraphs of fake portfolio-company content and Moderate paraphrasing meaning-changing sentences, so Assertive was retired and Moderate was rescoped to format-only on 2026-05-10. New axis: **raw / light / moderate**. Captain's Log retained as orthogonal fun preset. Old settings + history rows auto-migrate via serde aliases (`assertive` → Moderate). Light + Moderate prompts also forbid completing trailing sentence fragments and executing dictated meta-instructions (both bench-graded failure modes). Default for new installs: Light.
1. ✅ **Nemotron Speech Streaming via sherpa-onnx — benched, no default swap** _(2026-05-21 — `5ecd9b6` + bench at [`docs/benchmarks/wave-6-phase-1-2026-05-21.md`](./docs/benchmarks/wave-6-phase-1-2026-05-21.md))._ NVIDIA Nemotron Speech Streaming en-0.6B is wired as an addressable STT engine via the new `stt::online_transducer` FFI wrapper over sherpa-onnx's online recognizer and ships behind the existing `parakeet-engine` Cargo feature. Pull with `pnpm download:model:mac nemotron`, pick "NVIDIA Nemotron Speech Streaming 0.6B" in Settings → Voice → Recognition. **Benchmark verdict:** 17× slower than `whisper:base.en` on M4 Max **and** materially worse raw transcript quality on OOV-heavy technical content (matches the NeMo model card's "not recommended for word-for-word / incomplete sentences" warning). Default STT stays `whisper:base.en`. Nemotron remains a selectable alternate for short PTT bursts where streaming-partial latency would matter — but that benefit isn't realized until Phase 1.5 lands. License: NVIDIA Open Model License Agreement (commercial OK, not Apache/MIT — add NOTICE entry the day this flips to a default).
   - Companion LLM bench: ✅ `qwen3:4b` / `qwen3:8b` / `qwen3:1.7b` added to the harness (`687eacf`), then graded post-thinking-mode fixes (`6dde9ee`). **Verdict:** every Qwen3 candidate fails the iron rule. `qwen3:4b` is 9.4× slower than `qwen2.5:7b` at median with 1/8 success rate under bench load (Ollama drops connections during back-to-back swaps); `qwen3:8b` is 2.6× slower and over-edits on Light; `qwen3:1.7b` is 2× slower than `qwen2.5:1.5b` AND adds preambles + paraphrases first-person into third-person. Category-level finding: hybrid Instruct/Thinking models are a 2-3× latency tax in this pipeline regardless of `think:false` — Ollama still generates reasoning tokens internally. **LLM defaults stay `qwen2.5:7b` / `qwen2.5:1.5b`** until an explicit non-thinking SKU is available.
     1.5. ⏳ **Nemotron production-side streaming integration.** Wire chunked `accept_waveform` + partial-result polling for the online recognizer in the production hot path so live-preview latency during PTT is measurable in real usage (not just batch decode). Today the engine mirrors the offline `SttEngine` shape; this is the change that unlocks Nemotron's actual selling point.
...
- ✅ **NSWorkspace dispatched to main thread** (2026-05-05). `NSWorkspace.frontmostApplication()` was called directly from the session daemon and from the quickpaste capture/restore path on background threads; macOS 15 raises `NSException` from those APIs off-main, which aborts the process via Rust's `__rust_foreign_exception` (foreign Obj-C exception). Now routed through `AppHandle::run_on_main_thread` with a 500ms ceiling. Confirmed via crash report `boothrflow-2026-05-05-181401.ips`.
- ✅ **Quickpaste palette refresh-on-focus** (2026-05-05). Pre-warmed webview meant `onMount` only fired once at startup, so the palette never showed dictations completed since the first open. Now refreshes recent on every window focus.
- ✅ **Default STT flipped to whisper-base.en** (2026-05-05). Parakeet TDT 0.6B v3 caught making three pronoun swaps on a 64s capture ("he did well" → "you did well" etc.); whisper-base.en got it right every time on the same WAV. Speed delta isn't worth the meaning errors. Parakeet stays selectable in Settings. **Update 2026-05-10:** the 2026-05-09 192s investor-letter capture flipped the picture for proper-noun-heavy content — Whisper-base substituted "Mementis" for "Momentus" (a portfolio company), while Parakeet got Momentus / OpenSpace / Deskpass / Gen Log all correct. Engine descriptions now point users with name-heavy dictations toward Parakeet; defaults unchanged pending more graded captures.
- ✅ **Bench grading UI shows raw-vs-formatted diff** (2026-05-10). Multiset word-diff highlights words present in `formatted` but not `raw`, surfacing LLM hallucinations and trailing-fragment completions during grading instead of requiring eyeball-diffs of two textareas. Pure JS, 8 unit tests, in `src/lib/services/word-diff.ts`.

#### Open Wave 6 small-fixes follow-ups

- **Auto-route long recordings to whisper.** Parakeet's main appeal is throughput; once we add streaming partials for Parakeet (Phase 1) the case for it on short/live input strengthens, but ≥30s captures should probably pin to whisper for accuracy regardless. Rough policy: if no streaming engine is loaded and audio_seconds > 30, route to whisper.
- **Add the 2026-05-05 21:31 capture to a regression suite for parakeet pronoun confusion.** The captured WAV (`captures/2026-05-05T21-31-17.158595+00-00-Chrome.wav`) is a known parakeet failure case worth re-grading any time we bump the engine.
