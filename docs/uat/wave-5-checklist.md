...
> **Wave 5g unblock (2026-05-02):** Parakeet works end-to-end. The
> probe (`cargo run --example parakeet_probe --features "real-engines
parakeet-engine"`) transcribes the bundle's own test wav in ~470ms.
> Three pieces had to land: sherpa-rs 0.6.8 + matched sys, a Python
> metadata-propagation step (encoder → decoder + joiner, plus an
> injected `context_size=2`), and a `model_type=""` config so
> sherpa-onnx auto-detects the NeMo TDT loader path. **Re-run
> `pnpm download:model:mac parakeet`** before this part — the
> updated download script auto-runs the propagator and pip-installs
> the `onnx` package if missing.
...
- [ ] _Expected:_ no `dictation:model-missing` event; the next
      dictation will use Parakeet.
