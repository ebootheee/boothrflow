...
## Phase 3 — Windows code signing (1-2 days)
...
## Phase 4 — Auto-update wiring (1 day)
...
- **Update channel selector** in Settings → About: `Stable` (default)
  ...
  manifest. Phase 6 makes use of this; Phase 4 just wires the
  plumbing.
...
- Cut v0.1.0-beta.1 → v0.1.0-beta.2.
...
1. **Phase 1 first.** Release infra is the only phase with no
   external dependencies. Can land on day 1 of the wave and
   immediately start producing (unsigned) tagged artifacts that
   verify the matrix build works.
2. **Phase 2 (macOS signing) before Phase 3.** Eric is on macOS, so
   the first signed release benefits him immediately. Windows
   signing can lag by a release if Azure Trusted Signing onboarding
   takes time.
3. **Phase 4 (auto-update) on the back of Phase 2.** As soon as
   we have one signed macOS release, wiring auto-update gives Eric
   the upgrade path on his daily driver.
4. **Phase 5 (onboarding)** can happen in parallel with Phase 3.
   It's pure FE/Tauri work, no signing dependency.
5. **Phase 6 (channels)** last. Depends on Phase 4 being solid.
