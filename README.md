...
| **Wave 5**: prompt builder + correction store + app-context + OCR scaffolding + prefix caching                                                       | Done — runtime OCR & Parakeet handed off in [`docs/waves/wave-5-context-aware-cleanup.md`](./docs/waves/wave-5-context-aware-cleanup.md) |
| **Wave 5b**: auto-learn correction coordinator (heuristic + opt-in toggle + structure) + OCR prompt-injection sanitizer                              | Done — macOS AX read + runtime OCR + Parakeet still in handoff doc                                                                       |
| **Wave 5c**: real macOS AX focused-field read + Parakeet TDT 0.6B v3 engine behind `parakeet-engine` feature                                         | Done — runtime macOS Vision OCR + Windows UIAutomation deferred to Wave 5d                                                               |
| **Wave 5d**: runtime macOS Vision OCR + Windows UIAutomation focused-field read + Parakeet streaming integration                                     | Next                                                                                                                                     |
