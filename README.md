...
**Status:** 0.6.0-alpha. Hot path (mic → Whisper / Parakeet / Nemotron STT → LLM cleanup → paste) works on Windows and macOS, with persistent history, quick-paste palette, in-app Settings panel, and a developer-mode bench harness for grading STT/LLM swaps. **Wave 5, Wave 6 Phase 0, and Wave 6 Phase 1 have landed on `main`** — structure-aggressiveness style picker (raw / light / moderate), Bluetooth-aware mic default, listen-pill NSPanel overlay (works over full-screen apps), Nemotron Speech Streaming engine wired through a custom sherpa-onnx online-recognizer FFI wrapper, Qwen3 LLM family bench-graded with `/no_think` + `think:false` thinking-mode handling. **Phase 1 bench verdict: no default swaps** — see [`docs/benchmarks/wave-6-phase-1-2026-05-21.md`](./docs/benchmarks/wave-6-phase-1-2026-05-21.md). Phase 1.5 (Nemotron live-preview streaming in the hot path), Phase 2 (parakeet.cpp), Phase 3 (bench N=3+median) still pending — plan at [`docs/waves/wave-6-engine-and-formatting.md`](./docs/waves/wave-6-engine-and-formatting.md). After Wave 6: production polish (Wave 7), then connectors + UI rebuild + privacy audit (Wave 8). See [`ROADMAP.md`](./ROADMAP.md).
...
| **Wave 5**: context-aware cleanup (app-context, OCR, correction store, auto-learn) + Parakeet TDT 0.6B engine + captures + in-app bench grading UI                  | Done — merged to `main`                                                                                                                                      |
| **Wave 6 Phase 0**: structure-aggressiveness style overhaul (raw / light / moderate / assertive) + Bluetooth-aware mic default + history dropdown UX + bench warmup | ✅ Shipped on `main` — see [CHANGELOG](./CHANGELOG.md#2026-05-05--wave-6-phase-0--small-fixes-sweep)                                                         |
| **Wave 6 Phase 1**: Nemotron Speech Streaming engine + Qwen3 LLM bench grader + thinking-mode handling                                                              | ✅ Shipped on `main` — bench verdict: no default swaps. See [`docs/benchmarks/wave-6-phase-1-2026-05-21.md`](./docs/benchmarks/wave-6-phase-1-2026-05-21.md) |
| **Wave 6 Phase 1.5**: Nemotron production-side chunked streaming for live preview                                                                                   | Pending — gates the actual Nemotron quality-vs-speed story for the streaming-engine slot                                                                     |
| **Wave 6 Phase 2+3**: parakeet.cpp evaluation, bench N=3+median, multi-capture aggregate                                                                            | Pending — plan at [`docs/waves/wave-6-engine-and-formatting.md`](./docs/waves/wave-6-engine-and-formatting.md)                                               |
| **Wave 7**: production polish — code signing, auto-update, onboarding wizard, beta/stable channels                                                                  | Next — plan at [`docs/waves/wave-7-production-polish.md`](./docs/waves/wave-7-production-polish.md) (6 phases, 6-9 days)                                     |
| **Wave 8**: connectors (Obsidian / HTTP / Slack / voice-trigger) + hyper-modern UI rebuild + privacy audit doc                                                      | Queued — plan at [`docs/waves/wave-8-connectors-ui-privacy.md`](./docs/waves/wave-8-connectors-ui-privacy.md)                                                |
| Future ideas: meeting mode, **iOS mobile companion (E2E + on-device local)**, plugin API, insights, voice commands, snippets, Linux port                            | Captured — see [ROADMAP](./ROADMAP.md#future-ideas-post-wave-8)                                                                                              |
